...
def send_rejection_email(recipient_email: str, organization_name: str, project_title: str, reason: str) -> bool:
    if not all([SMTP_EMAIL, SMTP_PASSWORD]):
        logger.error("[EMAIL] SMTP credentials not configured!")
        return False
    if SMTP_EMAIL == "ton.email@gmail.com" or SMTP_PASSWORD == "ton-app-password-gmail":
    ...
    logger.info(f"[EMAIL] Sending rejection email to {recipient_email}")
...
<body style="margin: 0; padding: 20px; background: #f3f4f6;">
    <div class="container">
        ...
        <div class="content">
            ...
            <p>Thank you for submitting your project <strong>"{project_title}"</strong> to the Regional AI Repository platform.</p>
            <p>After review, we regret to inform you that your project submission has been <strong style="color:#DC2626;">rejected</strong>.</p>
            <p><strong>Reason for rejection:</strong></p>
            <div class="reason-box">
                <p>{reason}</p>
            </div>
            <p>You are welcome to revise your project based on the feedback above and resubmit it for review.</p>
    ...
    msg = MIMEMultipart("alternative")
    msg["Subject"] = f"Project Rejected: {project_title} - Regional AI Repository"
    msg["From"] = f"Regional AI Repository <{SMTP_EMAIL}>"
    msg["To"] = recipient_email
    msg.attach(MIMEText(html_body, "html"))
    ...
    try:
        ...
        with smtplib.SMTP(SMTP_SERVER, SMTP_PORT, timeout=10) as server:
            ...
            server.login(SMTP_EMAIL, SMTP_PASSWORD)
            server.sendmail(SMTP_EMAIL, recipient_email, msg.as_string())
        logger.info(f"[EMAIL] Rejection email sent successfully to {recipient_email}")
        return True
    except Exception as e:
        logger.error(f"[EMAIL] Failed to send rejection email: {type(e).__name__}: {e}")
        return False
